...
- Electron、electron-vite、React、SQLite 驱动和构建工具版本尚未选择；阶段 1/2 依据当前官方兼容信息和实际运行锁定。
- `better-sqlite3` 只是优先候选；必须分别通过 Electron 开发运行时和 Windows 打包产物验证。
- Windows Shell 宿主方案、native helper 形态与透明材料方案等待阶段 1 spike，不在纸面架构中假定可行。
- `docs/CONTRACTS.md` 按阶段 2 创建；在此之前不冻结具体 IPC 名称或数据库 schema。
